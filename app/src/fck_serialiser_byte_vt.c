...
{
	SDL_assert(s->vt == fck_byte_writer_vt || s->vt == fck_byte_reader_vt);
}
...
void fck_read_byte_i8(fck_serialiser *s, fck_serialiser_params* p, fckc_i8 *v, fckc_size_t c)
{
	...
	SDL_assert((((fck_memory_serialiser*)s)->at + sizeof(*v) * c) <= ((fck_memory_serialiser*)s)->capacity && "Out of bounds read");

	SDL_memcpy(v, ((fck_memory_serialiser*)s)->bytes + ((fck_memory_serialiser*)s)->at, c);
