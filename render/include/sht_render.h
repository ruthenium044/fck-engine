...
typedef struct sht_swapchain_state {
	sht_image_view view;
...
typedef struct sht_swapchain_vt
{
	...
	sht_swapchain_state (*wait_and_acquire)(sht_swapchain swapchain);
	sht_extent     (*extent)(sht_swapchain swapchain);
	sht_extent     (*display)(sht_swapchain swapchain);
	float          (*scale)(sht_swapchain swapchain);
	sht_bool32     (*is_ok)(sht_swapchain swapchain, const sht_swapchain_state* state);
...
{
	sht_render_pass (*begin)(sht_command_buffer command_buffer, const sht_render_desc *desc);
