...
void fck_write_string_i8(fck_serialiser *s, fck_serialiser_params *p, fckc_i8 *v, fckc_size_t c)
{
	...
	for (fckc_i8 *end = v + c; v != end; v++)
	{
		...
		int result = std->io->snprintf((char *)at, ((fck_memory_serialiser *)s)->capacity - ((fck_memory_serialiser *)s)->at, "%llu\n",
		                               (fckc_u64)*v);
...
void fck_write_string_f32(fck_serialiser *s, fck_serialiser_params *p, float *v, fckc_size_t c)
{
	...
	{
		fckc_u8 *at = ((fck_memory_serialiser *)s)->bytes + ((fck_memory_serialiser *)s)->at;
		int result = std->io->snprintf((char *)at, ((fck_memory_serialiser *)s)->capacity - ((fck_memory_serialiser *)s)->at, "%f\n", *v);
		assert(result >= 0);
...
static fckc_size_t fck_read_string_number_length(fck_serialiser *s)
{
	...
	{
		if (!std->character->isspace(((fck_memory_serialiser *)s)->bytes[index]))
		{
			if (std->character->isdigit(((fck_memory_serialiser *)s)->bytes[index]))
			{
				continue;
			}
