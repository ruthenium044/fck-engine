...
#define fck_input_source_keyboard_support_device_changes 0
...
#define fck_input_pkeyboard_owner_capacity 4
...
static fckc_size_t fck_input_pkeyboard_owners(fckc_u64 **owners);
static fckc_size_t fck_input_pkeyboard_events(fck_input_event *events, fckc_size_t size);
static fckc_size_t fck_input_pkeyboard_descriptions(fck_input_description **descriptions);
static fckc_size_t fck_input_pkeyboard_state(fckc_u64 owner, fck_input_state *states, fckc_size_t size);

typedef struct fck_input_source_pkeyboard
{
	fck_input_source source;

	fckc_u64 owners[fck_input_pkeyboard_owner_capacity];
	fck_input_description descriptions[fck_pkey_count];
	fck_input_data data[fck_pkey_count];
} fck_input_source_pkeyboard;

static fck_input_source_pkeyboard input_pkeyboard = (fck_input_source_pkeyboard){
	.source =
		(fck_input_source){
			.name = "keyboard",
			.owners = fck_input_pkeyboard_owners,
			.events = fck_input_pkeyboard_events,
			.descriptions = fck_input_pkeyboard_descriptions,
			.state = fck_input_pkeyboard_state,
		},
	.owners = {0},
	.descriptions =
		{
...
FCK_EXPORT_API fck_input_source *input_source_keyboard = &input_pkeyboard.source;

static fckc_size_t fck_input_pkeyboard_owners(fckc_u64 **owners)
{
	*owners = input_pkeyboard.owners;
	return 1;
}
...
static fckc_size_t fck_input_pkeyboard_events(fck_input_event *events, fckc_size_t size)
{
	...
	fckc_u32 max_event = fck_input_source_keyboard_support_device_changes ? SDL_EVENT_KEYBOARD_REMOVED : SDL_EVENT_KEY_UP;
	...
	for (;;)
	{
		...
		for (fckc_size_t index = 0; index < to_size_t(result); index++)
		{
			...
			case SDL_EVENT_KEY_DOWN:
				event->description = &input_pkeyboard.descriptions[e->key.scancode];
			...
			event->source = &input_pkeyboard.source;
			event->userdata = NULL;

			input_pkeyboard.data[event->description->id] = event->data;
		}
...
{
	// We exclude "none"
	*descriptions = input_pkeyboard.descriptions + 1;
...
static fckc_size_t fck_input_pkeyboard_state(fckc_u64 owner, fck_input_state *states, fckc_size_t size)
{
	for (fckc_size_t index = 0; index < size; index++)
	{
		...
		if (state->id >= fck_pkey_count)
		{
			fck_input_state *last_state = states + size - 1;
			*state = *last_state;
			last_state->id = fck_pkey_unknown;
			size = size - 1;
			index = index - 1;
			continue;
		}

		const fck_input_data *data = input_pkeyboard.data + state->id;
		state->data = *data;
	}
	return size;
}
